...
class Player {
  ...
  get canJump() {
    ...
    if (
      ...
      abs(this.x - this.lastWallStick.x) > COYOTE_RADIUS_WALLJUMP
    ) {
      return false;
    }
  ...
  subCycle(e) {
    ...
    if (G.isGravityInversed) {
      ...
      if (holdingJump && !this.jumpReleased) {
        const jumpHoldRatio =
          min(this.jumpHoldTime, MAX_JUMP_HOLD_TIME) / MAX_JUMP_HOLD_TIME;
      ...
      if (this.isRising) {
        ...
        this.y =
          easeOutQuad(jumpRatio) * (this.jumpEndY - this.jumpStartY) +
          this.jumpStartY;
      } else {
        // Fall down
        const gravity =
          this.sticksToWall && this.vY > 0
            ? WALL_GRAVITY_ACCELERATION
      ...
      const horizontalAcceleration = this.landed
        ? PLAYER_HORIZONTAL_FLOOR_ACCELERATION
        : PLAYER_HORIZONTAL_FLIGHT_ACCELERATION;
      this.vX += limit(
