...
class Game {
  constructor() {
    ...
    G.bandanaTrail = Array(
      ~~(MAX_BANDANA_LENGTH / MAIN_MENU_BANDANA_X_INTERVAL)
    )
      .fill(0)
      .map((x, i) => {
        return {
          x:
            G.bandanaSource.x +
            PLAYER_RADIUS / 2 +
            i * MAIN_MENU_BANDANA_X_INTERVAL,
        };
      });
  ...
  startAnimation() {
    ...
    G.centerLevel(G.level.index, 5, 0.5, () => {
  ...
  endAnimation() {
    ...
    const keyPrefix = nomangle(
      `OS13kTrophy,GG,${document.title},Beat the game - `
    );
  ...
  render() {
    ...
    if (DEBUG && getDebugValue("zoom")) {
      translate(
        -mousePosition.x + CANVAS_WIDTH / 2,
    ...
    if (G.clock % THUNDER_INTERVAL < 0.3) {
      ...
      R.lineWidth = 4;
      let x =
        createNumberGenerator(G.clock / THUNDER_INTERVAL).floating() *
        CANVAS_WIDTH;
      beginPath();
    ...
    BUILDINGS_BACKGROUND.forEach((layer, i) =>
      wrap(() => {
        ...
        translate(
          0,
          ~~(CANVAS_HEIGHT - layer.height + altitudeRatio * layerRatio * 400)
        );
    ...
    wrap(() => {
      translate(
        LEVEL_X,
        ~~G.bottomScreenAltitude + LEVEL_HEIGHT + TOWER_BASE_HEIGHT
      );
      ...
      wrap(() => {
        ...
        [30, 90, 150, 210].forEach((x) =>
          wrap(() => {
            R.globalAlpha =
              (sin((G.clock * PI * 2) / 2) * 0.5 + 0.5) * 0.1 + 0.2;
        ...
        wrap(() => {
          ...
          G.bandanaTrail.forEach((item, i, arr) => {
            ...
            item.y =
              G.bandanaSource.y -
              ratio * 30 +
              sin(-ratio * 20 + G.clock * 35) * amplitude;
          });
          ...
          renderCharacter(R, G.clock, PLAYER_BODY, true, -1, 0, 0);
        });
      });
      ...
      for (
        let i = max(0, currentLevelIndex - 1);
        i < min(LEVELS.length, currentLevelIndex + 2);
        i++
      ) {
        wrap(() => {
    ...
    wrap(() => {
      ...
      translate(
        (CANVAS_WIDTH * 3) / 8,
        CANVAS_HEIGHT + MOBILE_CONTROLS_HEIGHT / 2
      );
      renderMobileArrow();
    });
    ...
    // HUD
    const hudItems = [[nomangle("DIFFICULTY:"), G.difficulty.label]];
    ...
    if (G.timer) {
      ...
      hudItems.push([
        nomangle("TIME") +
          (G.wasDifficultyChangedDuringRun ? nomangle(" (INVALIDATED):") : ":"),
        formatTime(G.timer),
      ]);
    }
    ...
    hudItems.forEach(([label, value], i) =>
      wrap(() => {
        if (DEBUG && getDebugValue("nohud")) {
          return;
        }
    ...
    fillText(
      nomangle("Gamepad: ") +
        (gamepads().length ? nomangle("yes") : nomangle("no")),
    ...
    wrap(() => {
      ...
      outlinedText(
        G.interTitle,
        CANVAS_WIDTH / 2,
        INTER_TITLE_Y + G.interTitleYOffset
      );
    });
