...
class Game {
  ...
  startAnimation() {
    ...
    interp(
      ...
      easeInOutCubic,
      () => {
        interp(this, "windowsAlpha", 1, 0, 1, 1, null);
  ...
  render() {
    ...
    //   const y =
    //     (startY + G.clock * speed + this.bottomScreenAltitude) %
    //     evaluate(CANVAS_HEIGHT + RAIN_DROP_LENGTH);
    ...
    wrap(() => {
      translate(
        LEVEL_X,
        this.bottomScreenAltitude + LEVEL_HEIGHT + TOWER_BASE_HEIGHT
      );
    ...
    if (DEBUG) {
      wrap(() => {
        ...
        const info = ["fps: " + G.fps, fpsGauge.join("")];
        let y = 40;
        info.forEach((info) => {
          fillText(info, 40, y);
          y += 40;
