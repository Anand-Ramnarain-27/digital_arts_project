if (DEBUG) {
  ...
  addEventListener(
    ...
    (event) => {
      ...
      const x =
        (CANVAS.width * (event.pageX - canvasCoords.left)) / canvasCoords.width;
      const y =
        (CANVAS.height * (event.pageY - canvasCoords.top)) /
        canvasCoords.height;
      ...
      if (getDebugValue("editor")) {
        ...
        if (row >= 0 && row < LEVEL_ROWS && col >= 0 && col < LEVEL_COLS) {
          G.level.definition.matrix[row][col] =
            !G.level.definition.matrix[row][col];
    ...
    (e) => {
      if (getDebugValue("editor") && e.keyCode == KEYBOARD_S) {
        prompt(
          "Level matrix",
          JSON.stringify(
            G.level.definition.matrix.map((row) => row.map((x) => !!x + 0))
          )
        );
      }
